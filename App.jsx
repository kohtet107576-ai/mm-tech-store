...
// History ကို ဖြုတ်ပြီး အောက်မှာ HistoryIcon အနေနဲ့ ကိုယ်တိုင် ဆောက်သုံးပါမယ်
...
// Custom Icon to avoid Lucide Version issues
const HistoryIcon = () => (
...
export default function App() {
  ...
  // Admin input handling with State (No getElementById)
  ...
  useEffect(() => { 
    fetch(SCRIPT_URL).then(res => res.json()).then(data => { if (Array.isArray(data)) setProducts(data); }).catch(e => console.error(e));
  }, []);
  ...
  const handleOrder = async () => {
    if (!editContact && !profile?.contact) { alert("Please enter contact!"); return; }
    setLoading(true);
    const orderData = {
  ...
  const BottomNav = () => (
    <nav className="fixed bottom-0 left-0 right-0 bg-[#0a192f]/95 border-t border-blue-900/20 p-5 flex justify-around items-center z-50 max-w-md mx-auto rounded-t-2xl">
      ...
      <div className="max-w-md mx-auto w-full min-h-screen flex flex-col relative bg-[#0a192f] border-x border-blue-900/10">
        ...
        {view === 'welcome' && (
          <div className="flex flex-col flex-1 items-center justify-center p-10 text-center">
            ...
            <button onClick={handleLogin} className="w-full bg-white text-black py-4 rounded-2xl font-black active:scale-95 transition-all">Login with Google</button>
          </div>
        )}
        ...
        {view === 'checkout' && (
          <div className="p-8 flex flex-col flex-1 pb-40 text-left">
            ...
            <input type="text" placeholder="Contact Info (Telegram/Phone)" className="w-full bg-[#112240] border border-blue-900/30 p-5 rounded-2xl text-white outline-none mb-8 focus:border-blue-500 transition-all" value={editContact || profile?.contact || ''} onChange={e => setEditContact(e.target.value)} />
        ...
        {view === 'order_success' && (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            <CheckCircle2 size={80} className="text-green-500 mb-8 animate-bounce" />
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
              {adminTab === 'orders' ? (
                <div className="space-y-4">
                  {allOrders.map(o => (
                    <div key={o.id} className="bg-[#112240] p-6 rounded-[2rem] border border-blue-900/30 shadow-md">
                      ...
                      {o.status === 'Pending' && (
                        <div className="mt-4 space-y-3">
                          <input 
                            type="text" 
                            placeholder="Deliver Code/Gmail" 
                  ...
                  {allMembers.map(m => (
                    <div key={m.uid} className="bg-[#112240] p-5 rounded-3xl flex flex-col gap-4 border border-blue-900/10">
                      <div className="flex items-center gap-4">
                        <img src={m.photoURL || LOGO_URL} className="w-12 h-12 rounded-2xl" alt="U"/><div className="flex-1"><h4 className="font-black text-sm">{m.name}</h4><p className="text-[10px] text-blue-500 uppercase font-bold tracking-widest">{m.tier}</p></div>
                      </div>
                      <div className="flex gap-2">
                        {['Standard', 'VIP', 'Reseller'].map(t => (
                          <button key={t} onClick={() => updateTier(m.uid, t)} className={`flex-1 py-2 rounded-xl text-[9px] font-black transition-all ${m.tier === t ? 'bg-blue-600' : 'bg-slate-800 text-slate-400'}`}>{t}</button>
                        ))}
                      </div>
                    </div>
                  ))}
          ...
          <div className="p-10 flex flex-col flex-1 items-center justify-center pb-40 text-center">
            ...
            {user ? (
              <div className="w-full max-w-sm flex flex-col items-center">
                ...
                <button onClick={() => signOut(auth)} className="text-red-500 text-xs font-black flex items-center gap-2 hover:opacity-80 transition-all"><LogOut size={20}/> Sign Out</button>
              </div>
            ) : (
