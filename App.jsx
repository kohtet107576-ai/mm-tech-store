...
import { ShoppingBag, ArrowLeft, CheckCircle2, Loader2, User, ShieldCheck, LogOut, Send, Save, Image as ImageIcon, History, Plus, X, Search, ShoppingCart, LogIn, Facebook, Share2, MessageCircle, ChevronRight, Trash2, Wallet } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="bg-[#050d1a] min-h-screen text-white font-sans max-w-lg mx-auto overflow-hidden">
      {view === 'home' && <><MainHeader /><div className="p-6">{/* PRODUCTS CONTENT HERE */}<BottomNav /></div></>}
      {view === 'admin_dash' && (
        <div className="p-6 pb-40 min-h-screen">
          <MainHeader />
          <div className="flex gap-2 my-6">
            ...
            <button onClick={() => { setAdminTab('logs'); setActiveMember(null); }} className="bg-blue-600 px-4 py-2 rounded-xl text-xs font-bold">Logs</button>
          </div>
          ...
          {adminTab === 'member_history' && activeMember && (
            <div className="bg-[#112240] p-6 rounded-3xl">
              <button onClick={() => { setActiveMember(null); setAdminTab('members'); }} className="mb-4 text-blue-500 text-xs flex items-center"><ArrowLeft size={16}/> Back</button>
              ...
              <div className="space-y-2">{sysLogs.filter(l => l.targetGmail === activeMember.email).map(l => (
                <div key={l.id} className="flex justify-between p-3 bg-black/20 rounded-lg text-xs"><span>{l.detail}</span><span className={l.amount > 0 ? 'text-green-500' : 'text-red-500'}>{l.amount} Ks</span></div>
              ))}</div>
            </div>
          )}
      ...
      {view === 'customer_dash' && (
        <div className="p-6 pb-40">
           ...
           <div className="space-y-4">{myOrders.map(o => (<div key={o.id} className="bg-[#112240] p-4 rounded-xl text-xs">{o.product} - {o.price} Ks</div>))}</div>
           ...
           <div className="space-y-2">{sysLogs.filter(l => l.targetGmail === user?.email).map(l => (
             <div key={l.id} className="p-3 bg-[#112240] rounded-lg text-xs flex justify-between"><span>{l.detail}</span><span className={l.amount > 0 ? 'text-green-500' : 'text-red-500'}>{l.amount} Ks</span></div>
           ))}</div>
           <BottomNav />
        </div>
      )}
