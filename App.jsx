...
export default function App() {
  ...
  // --- WELCOME SCREEN (FIXED SPACING & COLORS) ---
  const WelcomeScreen = () => (
    <div className="flex flex-col h-[100dvh] bg-[#0a192f] text-center overflow-hidden border-x border-blue-900/20">
      <div className="flex flex-col items-center justify-between h-full py-12 px-8 text-white">
    ...
    <div className="flex flex-col h-[100dvh] bg-[#0a192f] overflow-hidden text-white">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 pb-32">
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
          {categories.map(c => (
            <button key={c.id} onClick={() => { setSelectedCat(c.id); setView('products'); }} className="bg-[#112240] p-5 rounded-[2rem] flex flex-col items-center gap-3 border border-transparent active:border-blue-500 transition-all text-white shadow-lg">
        ...
        <div className="space-y-4">
          {products.length > 0 ? (
            products.filter(p => searchQuery === '' || getVal(p, 'Name')?.toLowerCase().includes(searchQuery.toLowerCase())).map(p => (
              <div key={getVal(p, 'ID') || Math.random()} onClick={() => { setSelectedProduct(p); setView('details'); }} className="bg-[#112240] p-4 rounded-3xl flex gap-4 border border-blue-900/20 active:scale-95 transition-all text-left shadow-md">
                <div className="w-16 h-16 bg-[#0a192f] rounded-2xl overflow-hidden flex-shrink-0 border border-blue-900/30">
                    <img src={formatImageUrl(getVal(p, 'Link'))} alt="" className="w-full h-full object-cover opacity-80" onError={(e) => { e.target.src = "https://placehold.co/100"; }} />
                </div>
                ...
                <ChevronRight className="self-center text-slate-500" size={18} />
              </div>
            ))
          ) : (
              ...
              <button key={getVal(p, 'ID') || Math.random()} onClick={() => { setSelectedProduct(p); setView('details'); }} className="w-full bg-[#112240] p-5 rounded-3xl flex items-center justify-between text-white border border-blue-900/20 active:bg-blue-900/40 transition-all text-left shadow-lg">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#0a192f] rounded-xl overflow-hidden border border-blue-900/30 flex-shrink-0">
                      <img src={formatImageUrl(getVal(p, 'Link'))} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-white truncate">{getVal(p, 'Name')}</h4>
      ...
      {view === 'details' && (
        <div className="flex flex-col h-full text-white bg-[#0a192f] overflow-hidden text-left">
          <div className="relative h-[40vh] bg-[#112240] flex-shrink-0">
            <img src={formatImageUrl(getVal(selectedProduct, 'Link'))} className="w-full h-full object-cover opacity-70" alt="" />
