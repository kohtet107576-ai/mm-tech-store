...
// --- CONFIGURATION ---
// လူကြီးမင်း၏ Google Apps Script Web App URL ကို အသင့်ထည့်သွင်းပေးထားပါသည်
...
export default function App() {
  ...
  // Sheet ထဲက header နာမည် Link (သို့) link နှစ်မျိုးလုံးကို ဖတ်ပေးခြင်း
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fallback: Connection မရလျှင် Demo ပြရန်
  ...
  const WelcomeScreen = () => (
    <div className="flex flex-col h-[100dvh] bg-[#0a192f] p-8 text-center items-center justify-between overflow-hidden border-x border-blue-900/20">
      <div className="flex flex-col items-center pt-8 animate-in fade-in zoom-in duration-700">
      ...
      <div className="grid grid-cols-2 gap-4 w-full max-w-sm mb-10">
        <a href="https://sonema.znnt.org/" target="_blank" rel="noreferrer" className="bg-[#112240] py-4 rounded-xl border border-blue-900/50 text-[10px] font-black text-blue-400 text-center uppercase tracking-widest active:scale-95 transition-transform shadow-lg">စုံ/မ စစ်ဆေးရန်</a>
        <a href="http://www.ceir.gov.mm" target="_blank" rel="noreferrer" className="bg-[#112240] py-4 rounded-xl border border-blue-900/50 text-[10px] font-black text-blue-400 text-center uppercase tracking-widest active:scale-95 transition-transform shadow-lg">CEIR စစ်ဆေးရန်</a>
      </div>
    </div>
  );
  ...
  const HomeScreen = () => (
    <div className="flex flex-col h-[100dvh] bg-[#0a192f] overflow-hidden">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-[#0a192f]/95 backdrop-blur-lg border-t border-blue-900/30 p-5 flex justify-around items-center z-50 shadow-2xl">
