  ...
  const MainHeader = () => (
    <div className="flex items-center justify-between p-4 bg-[#0a192f] border-b border-blue-900/20 sticky top-0 z-40">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-md font-black text-white uppercase">MM Tech</h2>
      </div>
      ...
      <div className="max-w-md mx-auto w-full min-h-screen flex flex-col relative bg-[#0a192f] border-x border-blue-900/10">
        ...
        {view === 'welcome' && (
          <div className="flex flex-col flex-1 items-center justify-center p-10 text-center">
            ...
            <h1 className="text-4xl font-black mb-10">MM Tech</h1>
            <button onClick={handleLogin} className="w-full bg-white text-black py-4 rounded-2xl font-black">Login with Google</button>
          </div>
        )}
          ...
          <>
            ...
            <div className="p-6 pb-40 text-left">
              ...
              <div className="flex gap-2 overflow-x-auto pb-6">
                <button onClick={() => setSelectedCat(null)} className={`px-5 py-2 rounded-full text-[11px] font-black border ${!selectedCat ? 'bg-blue-600' : 'bg-[#112240]'}`}>All</button>
                {dynamicCategories.map(c => (
                  <button key={c.id} onClick={() => setSelectedCat(c.id)} className={`px-5 py-2 rounded-full text-[11px] font-black border ${selectedCat === c.id ? 'bg-blue-600' : 'bg-[#112240]'}`}>{c.name}</button>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-4">
                {groupedProducts.filter(g => !selectedCat || g.category === selectedCat).map(group => (
                  <div key={group.name} onClick={() => { setSelectedGroup(group); setView('group_details'); }} className="bg-[#112240]/40 p-3 rounded-2xl border border-blue-900/10">
              ...
              <div className="space-y-3">
                {selectedGroup?.plans.map((p, i) => (
                  <button key={i} onClick={() => { setSelectedPlan(p); setView('checkout'); }} className="w-full bg-[#112240] p-5 rounded-3xl border border-blue-900/20 flex items-center justify-between">
                    <div><h4 className="text-sm font-black">{getPProp(p, 'Plan')}</h4><p className="text-blue-500 font-black">{getDisplayPrice(p)} Ks</p></div>
        ...
        {view === 'checkout' && (
          <div className="p-8 flex flex-col flex-1 pb-40 text-left">
            ...
            <div className="bg-[#112240] p-10 rounded-[3rem] text-center mb-8">
            ...
            <input type="text" placeholder="Contact (Telegram/Phone)" className="w-full bg-[#112240] border border-blue-900/30 p-5 rounded-2xl text-white outline-none mb-8" value={editContact || profile?.contact || ''} onChange={e => setEditContact(e.target.value)} />
            <button onClick={handleOrder} className="w-full bg-blue-600 py-5 rounded-2xl font-black text-white">Confirm Order</button>
            <BottomNav />
          </div>
        )}
                  ...
                  {allOrders.map(o => (
                    <div key={o.id} className="bg-[#112240] p-6 rounded-[2rem] border border-blue-900/30">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase ${o.status === 'Completed' ? 'text-green-500' : 'text-yellow-500'}`}>{o.status}</span>
                      </div>
                      {o.status === 'Pending' && (
                  ...
                  {allMembers.map(m => (
                    <div key={m.uid} className="bg-[#112240] p-5 rounded-3xl flex flex-col gap-4">
                      <div className="flex items-center gap-4">
                        <img src={m.photoURL || LOGO_URL} className="w-12 h-12 rounded-2xl" alt="U"/><div className="flex-1"><h4 className="font-black text-sm">{m.name}</h4><p className="text-[10px] text-blue-500 uppercase">{m.tier}</p></div>
                      </div>
                ...
                <div key={o.id} className="bg-[#112240] p-6 rounded-[2rem] border border-blue-900/30">
                  ...
                  {o.status === 'Completed' && o.result && (
                    <div className="bg-blue-600/10 p-4 rounded-2xl flex items-center justify-between gap-3 mt-2">
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(o.result); alert("Copied!"); }} className="bg-blue-600 p-2 rounded-lg text-white"><Save size={14}/></button>
                    </div>
                  )}
          ...
          <div className="p-10 flex flex-col flex-1 items-center justify-center pb-40 text-center">
            ...
            {user ? (
              <div className="w-full max-w-sm flex flex-col items-center">
                <img src={user.photoURL} className="w-28 h-28 rounded-[2.5rem] border-4 border-blue-600 p-1 mb-6" alt="U"/>
                <h2 className="text-3xl font-black mb-1">{profile?.name}</h2>
                <div className="bg-[#112240] w-full p-8 rounded-[3rem] border border-blue-900/30 text-left mt-4 mb-10">
                  <div className="flex justify-between py-4 border-b border-blue-900/10 text-xs"><span className="text-slate-400 font-black">Tier</span><span className="text-blue-500 font-black">{profile?.tier}</span></div>
                  <div className="flex justify-between py-4 text-xs"><span className="text-slate-400 font-black">Contact</span><span className="font-bold">{profile?.contact || 'Not Set'}</span></div>
                </div>
                <button onClick={() => signOut(auth)} className="text-red-500 text-xs font-black flex items-center gap-2"><LogOut size={20}/> Sign Out</button>
              </div>
            ) : (
              <button onClick={handleLogin} className="bg-white text-black px-10 py-4 rounded-2xl font-black">Login</button>
            )}
            <BottomNav />
          </div>
        )}
