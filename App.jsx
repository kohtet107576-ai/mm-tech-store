...
export default function App() {
  ...
  // Categories data without JSX to prevent React child errors
  const categories = [
    { id: 'Game', name: 'Game', iconType: 'gamepad' },
    { id: 'Digital product', name: 'Digital Product', iconType: 'smartphone' },
    { id: 'Online class', name: 'Online Class', iconType: 'book' },
    { id: 'Gsm reseller', name: 'GSM Reseller', iconType: 'settings' }
  ];
  ...
  // Enforce string return to avoid "Objects are not valid as React child" error
  const getVal = (obj, key) => {
    if (!obj) return "";
    const val = obj[key] || obj[key.toLowerCase()] || obj[key.toUpperCase()] || obj[key.charAt(0).toUpperCase() + key.slice(1)];
  ...
  // Get current category data for safe rendering
  const currentCategoryProducts = products.filter(p => getVal(p, 'Category') === selectedCat);
  const searchedProducts = products.filter(p => searchQuery === '' || getVal(p, 'Name').toLowerCase().includes(searchQuery.toLowerCase()));

  // Payment details calculation
  const catName = selectedProduct ? getVal(selectedProduct, 'Category') : "";
  const kpayAccount = (catName.includes('Game') || catName.includes('Digital')) 
    ? "09793655312 (Sai Khun Thet Hein)" 
    : "09402021942 (Hnin Pwint Phyu)";

  // Layout Wrapper -> Direct inline JSX prevents remount and object errors
  return (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-8 pb-32 custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                {categories.map(cat => (
                  ...
                  >
                    <div className="p-3 bg-blue-600/20 text-blue-400 rounded-2xl">{getIcon(cat.iconType)}</div>
              ...
              <div className="space-y-4">
                {products.length > 0 ? searchedProducts.map(p => (
                  <div 
            ...
            <div className="flex-1 overflow-y-auto p-8 -mt-8 bg-[#0a192f] rounded-t-[2.5rem] relative shadow-[0_-10px_40px_rgba(0,0,0,0.3)] custom-scrollbar">
              <div className="flex justify-between items-start mb-6">
                <div className="overflow-hidden pr-2">
                  <span className="px-3 py-1 bg-blue-600/20 border border-blue-500/20 text-blue-400 rounded-full text-[10px] font-black uppercase tracking-widest inline-block mb-3">{catName}</span>
              ...
              <div className="p-5 bg-[#112240] rounded-3xl border border-blue-900/50 mb-8">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-[#0a192f] p-4 rounded-2xl border border-blue-900/50 text-center shadow-inner">
                    ...
                    <span className="text-sm font-black text-blue-400">{kpayAccount}</span>
                  </div>
                </div>
              </div>
